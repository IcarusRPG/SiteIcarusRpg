      ...
      <div className="container-icarus">
        <div className="mx-auto max-w-6xl overflow-hidden rounded-[2rem] border border-gold/25 bg-black/40 shadow-gold backdrop-blur-md">
          <div className="grid gap-0 lg:grid-cols-[0.9fr_1.1fr]">
            <div className="relative min-h-[24rem] border-b border-gold/20 bg-abyss/60 lg:border-b-0 lg:border-r">
              <Image
            ...
            <article className="relative p-7 sm:p-10 lg:p-12">
              <div className="absolute right-0 top-0 h-48 w-48 rounded-full bg-mystic/10 blur-3xl" />
              <p className="font-display text-sm uppercase tracking-[0.35em] text-mystic/85">Sobre Icarus</p>
              <h1 className="mt-3 font-display text-4xl font-black text-ember sm:text-5xl">Bem-vindo ao mundo de Icarus</h1>
              <div className="gold-divider my-8" />

              <div className="space-y-5 text-base leading-8 text-parchment/82">
                {paragraphs.map((paragraph, index) => (
                  <p key={paragraph} className={`whitespace-pre-line ${index === 0 ? "first-letter:float-left first-letter:mr-3 first-letter:font-display first-letter:text-7xl first-letter:font-black first-letter:leading-[0.8] first-letter:text-ember first-letter:drop-shadow-[0_0_12px_rgba(216,163,58,0.45)]" : ""}`}>
                    {paragraph}
                  </p>
                ))}
              </div>

              <aside className="mt-10 rounded-3xl border border-mystic/25 bg-mystic/5 p-6 shadow-arcane">
