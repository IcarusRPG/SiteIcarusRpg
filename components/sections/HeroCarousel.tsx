...
export default function HeroCarousel() {
  ...
  return (
    <div className="group relative mx-auto mb-8 aspect-[16/9] w-full max-w-5xl overflow-hidden rounded-[2rem] border border-mystic/20 bg-black/35 shadow-arcane backdrop-blur-sm">
      ...
      <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-2 rounded-full border border-mystic/20 bg-black/45 px-3 py-2 backdrop-blur-md">
        {carouselImages.map((image, index) => (
          <button
            key={image.src}
            type="button"
            ...
            className={`h-2.5 rounded-full transition-all duration-300 ${
              activeIndex === index ? "w-8 bg-ember shadow-gold" : "w-2.5 bg-parchment/45 hover:bg-mystic"
            }`}
