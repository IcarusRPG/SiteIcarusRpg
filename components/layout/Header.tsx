...
export default function Header() {
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-gold/20 bg-abyss/75 shadow-[0_8px_30px_rgba(0,0,0,0.35)] backdrop-blur-xl">
      <div className="container-icarus flex h-20 items-center justify-between gap-4">
        <Link href="/" className="group flex items-center gap-3" aria-label="Voltar para a página principal de Icarus RPG">
          <Image
            ...
            height={120}
            priority
            className="h-12 w-auto max-w-[9.5rem] object-contain drop-shadow-[0_0_12px_rgba(16,216,207,0.25)] transition duration-300 group-hover:scale-105"
          />
        </Link>
          ...
          {navigationItems.map((item) =>
            item.children ? (
              ...
              <Link
                key={item.href}
                href={item.href}
                className="rounded-full px-3 py-2 text-sm font-medium text-parchment/90 transition duration-300 hover:bg-mystic/10 hover:text-ember"
              >
                {item.label}
              </Link>
            ),
